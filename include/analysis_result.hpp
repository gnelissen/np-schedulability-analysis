...
void print_result(const std::string& fname, const Analysis_result& result, 
	...
	std::cout << ",  " << result.number_of_jobs
				...
				if (config.want_task_chains)
					std::cout << ", " << (int) result.task_chains_analysis_success;
				else
					std::cout << ", -";
...
Analysis_result get_analysis_result(const std::unique_ptr<NP::Global::State_space<Time>>& space, const NP::Scheduling_problem<Time>& problem, const Analysis_config& config) {
	...
	if (config.want_mk) {
		...
		result.mk_analysis_success = space->template is_success<NP::Global::MK_analysis::MK_sp_data_extension<Time>>();
	}
	if (config.want_task_chains) {
		...
		result.task_chains_analysis_success = space->template is_success<NP::Global::Taskchains_analysis::Taskchains_sp_data_extension<Time>>();
	}
#endif		
	return result;
}
