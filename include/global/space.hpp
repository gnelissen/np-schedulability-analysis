...
namespace NP {
	...
	namespace Global {
		...
		public:
			...
			typedef std::shared_ptr<State> Cluster_ref;
		...
		private:
			...
			const std::vector<std::vector<Interval<Time>>> cores_initial_state;
...
#endif
			)	: state_space_data(jobs, edges, aborts, cores_initial_state)
				, aborted(false)
				, timed_out(false)
		...
		private:
				...
				// to miss their deadline
						...
						it != state_space_data.jobs_by_deadline[i].end(); it++)
					{
						...
						if (j.get_deadline() <= latest) {
							...
							// deadlines now after the next latest availability time
							break;
					}
				}
			}
			...
			{
				return n.job_not_dispatched(j.get_job_index()) && n.job_ready(state_space_data.predecessors_of(j));
			}
			...
			void explore(const Node_ref& n)
			{
				...
				for (int i = 0; i < num_clusters; i++) {
					...
					auto avail_max = n->latest_core_availability(i);
